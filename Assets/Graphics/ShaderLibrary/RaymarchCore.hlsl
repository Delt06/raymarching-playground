...
float ray_march(const float3 ro, const float3 rd, const float max_dist)
{
    float d_o = 0;
...
inline float ray_march(const float3 ro, const float3 rd)
{
    return ray_march(ro, rd, MAX_DIST);
}
...
#define RAY_MARCH(ro, hit_pos) const float3 rd = normalize(hit_pos - ro); const float d = ray_march(ro, rd);\
